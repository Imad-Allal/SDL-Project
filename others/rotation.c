...
void rotate (int x, int y, int cx, int cy, double angle){
    int x1, y1;
    points2[n].x = cx + (x - cx) * cos(angle) - (y - cy) * sin(angle);
    points2[n].y = cy + (x - cy) * sin(angle) + (y - cy) * cos(angle);
}
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i<N; i++){
        rotate(points[i].x, points[i].y, cx, cy, 0.1259);
        n++;
    ...
    for (int i = 0; i<N; i++){
        if(i == N-1){ 
            algoNaif(points2[i].x, points2[i].y, points2[0].x, points2[0].y);
        }
        else
            algoNaif(points2[i].x, points2[i].y, points2[i+1].x, points2[i+1].y);
    }    
    SDL_RenderPresent(renderer);
